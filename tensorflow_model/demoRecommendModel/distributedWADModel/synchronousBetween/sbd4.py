...
def WAD_model(x, y_actual, batch_fem, feature_length, embedding_length, hidden_layer, keep_prob, learning_rate):
    ...
    optimizer = tf.train.SyncReplicasOptimizer(opt, 2, total_num_replicas=2)
    global_step = tf.train.get_or_create_global_step()
    train = optimizer.minimize(mean_loss, global_step=global_step)
    sync_hook = optimizer.make_session_run_hook((FLAGS.task_index == 0))
    return y_predict, mean_loss, train, global_step, sync_hook
...
def main(_):
    ...
    if FLAGS.job_name == "ps":
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index, cluster=cluster)):
            ...
            # 模型
            y_predict, mean_loss, train, global_step, sync_hook = WAD_model(x, y_actual, batch_fem, feature_length,
                                                                 embedding_length, [15, 10, 5], keep_prob, 0.001)
            auc_reset, auc_value = tf.metrics.auc(y_actual, y_predict)

            hooks = [sync_hook, tf.train.StopAtStepHook(last_step=500)]
            checkpoint_dir = "..\distributedModelSavePath\SyBetWADModel"
...
if __name__ == "__main__":
    ...
    tf.app.flags.DEFINE_integer("task_index", 1, "job的task索引")
    tf.app.run()