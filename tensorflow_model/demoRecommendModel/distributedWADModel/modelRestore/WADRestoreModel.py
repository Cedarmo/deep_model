...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        saver = tf.train.import_meta_graph("..\distributedModelSavePath\SyBetWADModel\model.ckpt-500.meta", clear_devices=True)
        saver.restore(sess, "..\distributedModelSavePath\SyBetWADModel\model.ckpt-500")

        graph = tf.get_default_graph()
        x_input = graph.get_operation_by_name("x_input").outputs[0]
        y_actual = graph.get_operation_by_name("y_actual").outputs[0]
        batch_fem = graph.get_operation_by_name("batch_fem").outputs[0]
        keep_prob = graph.get_operation_by_name("keep_prob").outputs[0]
        feed_dict_test_data = {x_input: test_data[:, 1:], batch_fem: np.ones([len(test_data), 16, 5]),
